...
export async function POST() {
  const fRes = await fetch(
    `${SUPABASE_URL}/rest/v1/faktury?stav=eq.zaplacena&zauctovano_platba=eq.false&select=*`,
  ...
  for (const f of faktury) {
    try {
      ...
      if (!abraFa?.id) {
        results.push({ id: f.id, ok: false, error: 'Faktura nenalezena v ABRA' })
        continue
      }

      // Find paired transakce for actual payment date
      ...
      results.push({ id: f.id, ok, error: bankaErr })
    } catch (e) {
      results.push({ id: f.id, ok: false, error: String(e) })
    }
  }

  return NextResponse.json({ ok: true, processed: faktury.length, results })
}
