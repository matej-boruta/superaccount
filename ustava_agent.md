...
- Explicitního pravidla v ústavě nebo `dodavatel_pravidla` → 90–100 %
...
VSTUP
  │
  ▼
Mám explicitní pravidlo z ústavy nebo dodavatel_pravidla?
  │ ANO → Použij (confidence = pravidlu) → LOG → AKCE
  │ NE
  ▼
Mám historický vzor (ucetni_vzory, confidence ≥ 70)?
...
- Claude API nedostupné → použije deterministická pravidla z `dodavatel_pravidla` bez AI
